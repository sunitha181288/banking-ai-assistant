...
import { Upload, FileText, X, Database, DownloadCloud, Search, Cpu, MessageSquare, ChevronDown, ChevronUp } from "lucide-react";
...
export default function Sidebar({ documents, setDocuments, memoryItems }) {
  ...
  const [expanded,  setExpanded]  = useState(false); // mobile collapse
    ...
    <>
      ...
      <div className="md:hidden bg-gray-900 border-b border-gray-700">
        <button
          ...
          className="flex items-center justify-between w-full px-4 py-3 text-sm font-medium text-yellow-400">
          <div className="flex items-center gap-2">
            <Database size={14} />
            Knowledge Base — {documents.length} doc{documents.length !== 1 ? "s" : ""}, {totalChunks} chunks
          </div>
              ...
              <div className="space-y-1.5">
                {documents.map((doc, i) => (
                  <div key={i} className="flex items-center gap-2 bg-gray-800 rounded-lg px-3 py-2">
                    ...
                    <span className="flex-1 text-xs text-gray-200 truncate">{doc.filename}</span>
                    <span className="text-xs text-green-400">{doc.chunks}c</span>
                    <button onClick={() => removeDoc(doc.filename)} className="text-gray-500 hover:text-red-400">
                      <X size={12} />
                    </button>
                  </div>
                ))}
      ...
      <aside className="hidden md:flex w-72 min-w-72 bg-gray-900 border-r border-gray-700 flex-col overflow-y-auto">
        ...
        <div className="p-4 border-b border-gray-700">
          ...
          <label className={`flex flex-col items-center gap-2 border-2 border-dashed rounded-xl p-4 text-center cursor-pointer transition-all ${
            dragOver ? "border-yellow-400 bg-yellow-900/10" : "border-gray-600 hover:border-yellow-600"
          }`}
            onDragOver={e => { e.preventDefault(); setDragOver(true); }}
            onDragLeave={() => setDragOver(false)}
            ...
            <div>
              <p className="text-sm text-gray-300 font-medium">{uploading ? "Uploading…" : "Drop files or click"}</p>
            ...
            <ul className="mt-3 space-y-1.5">
              {documents.map((doc, i) => (
                <li key={i} className="flex items-center gap-2 bg-gray-800 rounded-lg px-3 py-2">
                  <FileText size={13} className="text-yellow-400 flex-shrink-0" />
                  <span className="flex-1 text-xs text-gray-200 truncate" title={doc.filename}>{doc.filename}</span>
                  <span className="text-xs text-green-400 font-medium">{doc.chunks}c</span>
                  <button onClick={() => removeDoc(doc.filename)} className="text-gray-500 hover:text-red-400 transition-colors">
                    <X size={12} />
                  </button>
                </li>
              ))}
        ...
        <div className="p-4 border-b border-gray-700">
          ...
          <div className="grid grid-cols-2 gap-2">
            {[{ label:"Documents", value:documents.length, Icon:FileText },
              { label:"Chunks",    value:totalChunks,      Icon:Database }].map(({label,value,Icon}) => (
              <div key={label} className="bg-gray-800 rounded-lg p-3 text-center">
                <div className="flex justify-center mb-1"><Icon size={13} className="text-yellow-400" /></div>
        ...
        {/* Memory */}
        <div className="p-4 border-b border-gray-700">
          <p className="text-xs font-semibold text-yellow-400 uppercase tracking-widest mb-3">Conversation Memory</p>
            ...
            : <ul className="space-y-1.5">
                {memoryItems.map((item, i) => (
                  <li key={i} className="bg-gray-800 rounded-md px-3 py-2 text-xs text-gray-400 border-l-2 border-yellow-600">
                    <span className="text-gray-200 font-medium">Q{memoryItems.length - i}: </span>{item}
                  </li>
                ))}
              </ul>
          }
        ...
        <div className="p-4">
          ...
          <div className="space-y-2.5 text-xs text-gray-400">
            {[{ Icon:DownloadCloud, label:"Ingest",   desc:"file split into chunks"  },
              ...
              { Icon:MessageSquare, label:"Remember", desc:"full history kept"        }].map(({Icon,label,desc}) => (
